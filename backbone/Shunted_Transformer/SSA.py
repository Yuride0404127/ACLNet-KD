...
def shunted_t(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load("/media/yuride/date/model/backbone/Shunted_Transformer/ckpt_T.pth"),
                             strict=False)
        print("Loaded shunted_t weights")
...
def shunted_s(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        model.load_state_dict(torch.load("/media/yuride/date/model/backbone/Shunted_Transformer/ckpt_S.pth"),
                             strict=False)
        print("Loaded shunted_s weights")
...
def shunted_b(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        model.load_state_dict(torch.load("/media/yuride/date/model/backbone/Shunted_Transformer/ckpt_B.pth"),
                             strict=False)
        print("Loaded shunted_b weights")

    return model


if __name__ == '__main__':
    input_rgb = torch.randn(2, 3, 224, 224)
    input_depth = torch.randn(2, 1, 224, 224)
    net = shunted_s(pretrained=True)
    out = net(input_rgb)
    print("out1", out[0].shape)
    print("out2", out[1].shape)
    print("out3", out[2].shape)
    print("out8", out[3].shape)